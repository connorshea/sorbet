...
class Sorbet::Private::GemLoader
  ...
  def self.require_all_gems
    ...
    # Do not load gems in Gemfile where `require` is false, this loads the gems
    # from the Gemfile because the lockfile parser doesn't surface information
    # about whether a gem is required or not.
    required_dependency_names = Bundler.load.dependencies.reject { |dep| dep.autorequire && dep.autorequire.empty? }.map(&:name)
